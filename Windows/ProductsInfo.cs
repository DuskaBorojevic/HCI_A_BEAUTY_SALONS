using HCI_A.Dao;
using HCI_A.Models.Enums;
using HCI_A.Models;
using System.Windows;
using System.Windows.Controls;
using System.Windows.Media;
using HCI_A.Windows;
using HCI_A.Helpers;

namespace HCI_A.Components.EmpoyeeSharedComponents
{
    public static class ProductsInfo
    {
        public static UIElement InitProducts(Window parentWindow, Employee employee)
        {
            BeautySalon salon = BeautySalonDao.GetBeautySalonById(employee.BeautySalonId);
            var products = ProductDao.GetProductsByPriceListId(salon.PriceList.PriceListId);

            StackPanel mainPanel = new StackPanel();

            Border productsCard = new Border();
            productsCard.SetResourceReference(Border.StyleProperty, "CardStyle");

            StackPanel cardContent = new StackPanel();

            Grid headerContainer = new Grid();
            headerContainer.Margin = new Thickness(0, 0, 0, 15);
            headerContainer.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            headerContainer.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });

            TextBlock titleBlock = new TextBlock
            {
                FontSize = 18,
                FontWeight = FontWeights.SemiBold,
                VerticalAlignment = VerticalAlignment.Center
            };
            titleBlock.SetResourceReference(TextBlock.TextProperty, "ProductsLabel");
            titleBlock.SetResourceReference(TextBlock.ForegroundProperty, "PrimaryBrush");
            Grid.SetColumn(titleBlock, 0);
            headerContainer.Children.Add(titleBlock);

            if (employee.AccountType == AccountType.MANAGER)
            {
                Button addProductButton = ButtonFactory.CreateAddButton();
                addProductButton.Click += (s, e) =>
                {
                    AddProductDialog dialog = new AddProductDialog(parentWindow, salon.PriceList.PriceListId);
                    dialog.ShowDialog();
                };
                Grid.SetColumn(addProductButton, 1);
                headerContainer.Children.Add(addProductButton);
            }

            cardContent.Children.Add(headerContainer);

            Grid tableHeader = new Grid
            {
                Margin = new Thickness(0, 0, 0, 10),
                Height=40
            };
            tableHeader.SetResourceReference(Grid.BackgroundProperty, "SecondaryLightBrush");

            for (int i = 0; i < 5; i++)
                tableHeader.ColumnDefinitions.Add(new ColumnDefinition());

            TextBlock selectLabel = CreateHeaderLabel(0);
            //TextBlock idLabel = CreateHeaderLabel(1);
            TextBlock nameLabel = CreateHeaderLabel(1);
            TextBlock priceLabel = CreateHeaderLabel(2);
            TextBlock descriptionLabel = CreateHeaderLabel(3);
            TextBlock availabilityLabel = CreateHeaderLabel(4);

            selectLabel.SetResourceReference(TextBlock.TextProperty, "SelectLabel");
            //idLabel.SetResourceReference(TextBlock.TextProperty, "IdLabel");
            nameLabel.SetResourceReference(TextBlock.TextProperty, "NameLabel");
            priceLabel.SetResourceReference(TextBlock.TextProperty, "PriceLabel");
            descriptionLabel.SetResourceReference(TextBlock.TextProperty, "DescriptionLabel");
            availabilityLabel.SetResourceReference(TextBlock.TextProperty, "AvailabilityLabel");

            tableHeader.Children.Add(selectLabel);
            //tableHeader.Children.Add(idLabel);
            tableHeader.Children.Add(nameLabel);
            tableHeader.Children.Add(priceLabel);
            tableHeader.Children.Add(descriptionLabel);
            tableHeader.Children.Add(availabilityLabel);

            cardContent.Children.Add(tableHeader);

            foreach (var product in products)
            {
                Grid row = new Grid
                {
                    Margin = new Thickness(0, 5, 0, 0),
                    Height = 40,
                    Cursor = System.Windows.Input.Cursors.Hand
                };
                row.SetResourceReference(Grid.BackgroundProperty, "CardBrush"); //ElevatedSurfaceBrush

                for (int i = 0; i < 5; i++)
                    row.ColumnDefinitions.Add(new ColumnDefinition());

                RadioButton radioButton = new RadioButton
                {
                    VerticalAlignment = VerticalAlignment.Center,
                    HorizontalAlignment = HorizontalAlignment.Center,
                    Tag = product
                };

                //TextBlock idBlock = CreateRowLabel(product.ProductId.ToString(), TextAlignment.Center);
                TextBlock nameBlock = CreateRowLabel(product.Name, TextAlignment.Left);
                TextBlock priceBlock = CreateRowLabel(product.Price.ToString("F2"), TextAlignment.Center);
                TextBlock descriptionBlock = CreateRowLabel(product.Description, TextAlignment.Left);
                descriptionBlock.TextWrapping = TextWrapping.Wrap;

                TextBlock availabilityBlock = new TextBlock
                {
                    VerticalAlignment = VerticalAlignment.Center,
                    HorizontalAlignment = HorizontalAlignment.Center,
                    Padding = new Thickness(5)
                };
                availabilityBlock.SetResourceReference(TextBlock.TextProperty, product.Availability ? "Available" : "Unavailable");
                availabilityBlock.SetResourceReference(TextBlock.ForegroundProperty, product.Availability ? "SuccessBrush" : "ErrorBrush");

                radioButton.Checked += (s, e) =>
                {
                    ProductInfoWindow productInfoWindow = new ProductInfoWindow(product, employee);
                    productInfoWindow.Closed += (sender, args) =>
                    {
                        radioButton.IsChecked = false;

                        availabilityBlock.SetResourceReference(TextBlock.TextProperty, product.Availability ? "Available" : "Unavailable");
                        availabilityBlock.SetResourceReference(TextBlock.ForegroundProperty, product.Availability ? "SuccessBrush" : "ErrorBrush");
                        priceBlock.Text = product.Price.ToString("F2");
                        descriptionBlock.Text = product.Description;
                    };
                    productInfoWindow.ShowDialog();
                };

                Grid.SetColumn(radioButton, 0);
                //Grid.SetColumn(idBlock, 1);
                Grid.SetColumn(nameBlock, 1);
                Grid.SetColumn(priceBlock, 2);
                Grid.SetColumn(descriptionBlock, 3);
                Grid.SetColumn(availabilityBlock, 4);

                row.Children.Add(radioButton);
                //row.Children.Add(idBlock);
                row.Children.Add(nameBlock);
                row.Children.Add(priceBlock);
                row.Children.Add(descriptionBlock);
                row.Children.Add(availabilityBlock);

                cardContent.Children.Add(row);
            }

            productsCard.Child = cardContent;
            mainPanel.Children.Add(productsCard);

            return mainPanel;
        }

        private static TextBlock CreateHeaderLabel(int column)
        {
            TextBlock label = new TextBlock
            {
                FontWeight = FontWeights.SemiBold,
                VerticalAlignment = VerticalAlignment.Center,
                HorizontalAlignment = HorizontalAlignment.Center,
                Padding = new Thickness(5)
            };
            label.SetResourceReference(TextBlock.ForegroundProperty, "PrimaryDarkBrush");
            Grid.SetColumn(label, column);
            return label;
        }

        private static TextBlock CreateRowLabel(string text, TextAlignment alignment)
        {
            TextBlock tb = new TextBlock
            {
                Text = text,
                VerticalAlignment = VerticalAlignment.Center,
                HorizontalAlignment = alignment == TextAlignment.Center ? HorizontalAlignment.Center : HorizontalAlignment.Left,
                Padding = new Thickness(5)
            };
            tb.SetResourceReference(TextBlock.ForegroundProperty, "TextBrush");
            return tb;
        }
    }
}